...
class OutlierRemover(OutlierHandler): 
    ...
    def handle_outliers(self, X_train: pd.DataFrame, y_train: pd.DataFrame):
        categorical_columns = list(set(X_train.columns) - set(config.numerical_columns))
        numerical_columns = list(set(X_train.columns) - set(config.categorical_columns))

        # remove the has_secondary_use and has_superstructure columns to not run analysis on them
        categorical_columns = list(
            set(categorical_columns) - set(config.has_superstructure_columns) - set(config.has_secondary_use_columns)
        )

        # rows we found to contain outliers which can therefore be dropped
        row_indizes_to_remove = handle_outliers.get_outlier_rows_as_index(
            X_train,
            numerical_columns,
            categorical_columns,
            self.cat_threshold,
